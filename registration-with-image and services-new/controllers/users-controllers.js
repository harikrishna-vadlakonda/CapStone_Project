...
const signup = async (req, res, next) => {
    ...
    const {firstname, lastname, mobilenumber, email, gender, city, password} = req.body;
    ...
    try {
        token = jwt.sign(
            {userId: createdUser.id, mobilenumber: createdUser.mobilenumber},
    ...
    await res
        .status(201)
        .json({userId: createdUser.id, mobilenumber: createdUser.mobilenumber, token: token});
};
...
const login = async (req, res, next) => {
    ...
    try {
        token = jwt.sign(
            {userId: existingUser.id, mobilenumber: existingUser.mobilenumber},
            'supersecret_dont_share',
        );
    } catch (err) {
        const error = new HttpError(
